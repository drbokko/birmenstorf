 ...
 *
 * Run after connect_and_configure_and_arm_detector (same DCU host). --ntrigger must match ntrigger.
...
int main(int argc, char *argv[]) {
    ...
    for (int i = 1; i < argc; i++) {
        if (std::strcmp(argv[i], "--help") == 0 || std::strcmp(argv[i], "-h") == 0) {
            std::printf("Usage: %s [--ntrigger N] [HOST]\n"
                        ...
                        "  --ntrigger N   number of triggers (must match ntrigger from part 1)\n",
                        argv[0]);
            return 0;
        }
        if (std::strcmp(argv[i], "--ntrigger") == 0) {
            if (i + 1 >= argc) {
