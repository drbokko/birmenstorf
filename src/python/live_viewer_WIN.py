...
class StreamReceiver(threading.Thread):
    ...
    def _setup_live(self):
        ...
        im = ax.imshow(np.zeros((10, 10), dtype=np.uint16), cmap='gray', vmin=0, vmax=1, animated=False, aspect='auto')
                ...
                if mtype == 'start':
                    ...
                    if self._im and draw_this:
                        ...
                        if self._im.get_array().shape != data.shape:
                            ...
                            # Dynamically adjust the aspect ratio based on the image dimensions
                            self._ax.set_aspect(data.shape[0] / data.shape[1])
