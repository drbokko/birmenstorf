...
/* Without this, libcurl writes the response body to stdout when CURLOPT_WRITEFUNCTION is unset. */
size_t eiger_curl_discard_cb(void *contents, size_t size, size_t nmemb, void *userp) {
    (void)contents;
    (void)userp;
    return size * nmemb;
}
...
int eiger_http_request(const char *host, int port,
    ...
    if (g_eiger_http_trace) {
        std::fprintf(g_eiger_http_trace, "[SIMPLON API] -> %s http://%s:%d%s", method, host, port, path);
        if (std::strcmp(method, "PUT") == 0)
    ...
    if (response && response_size > 0) {
        ...
        /* Drain body when caller does not want it (avoids leaving data on the connection). */
        char discard[4096];
        DWORD dwDownloaded = 0;
        for (;;) {
            DWORD dwSize = 0;
            if (!WinHttpQueryDataAvailable(hRequest, &dwSize) || dwSize == 0)
                break;
            DWORD toRead = dwSize > sizeof(discard) ? static_cast<DWORD>(sizeof(discard)) : dwSize;
            if (!WinHttpReadData(hRequest, discard, toRead, &dwDownloaded) || dwDownloaded == 0)
                break;
        }
    }
    ...
    if (response && response_size > 0) {
        ...
        curl_easy_setopt(curl, CURLOPT_WRITEFUNCTION, eiger_curl_discard_cb);
